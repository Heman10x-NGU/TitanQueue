...
import (
	...
	titanqueuecontext "github.com/hemant/titanqueue/internal/context"
...
func (p *processor) exec() {
	...
	case p.sema <- struct{}{}: // acquire token
		...
		go func() {
			...
			ctx, cancel := titanqueuecontext.New(p.baseCtxFn(), msg, deadline)
...
// sortByPriority returns a list of queue names sorted by their priority level in descending order.
...
func (p *processor) computeDeadline(msg *base.TaskMessage) time.Time {
	if msg.Timeout == 0 && msg.Deadline == 0 {
		p.logger.Errorf("titanqueue: internal error: both timeout and deadline are not set for the task message: %s", msg.ID)
