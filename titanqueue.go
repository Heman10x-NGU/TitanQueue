...
/*
Package titanqueue provides a framework for Redis based distributed task queue.

TitanQueue uses Redis as a message broker. To connect to redis,
specify the connection using one of RedisConnOpt types.

	redisConnOpt = titanqueue.RedisClientOpt{
	    Addr:     "127.0.0.1:6379",
	    Password: "xxxxx",
	    DB:       2,
	}
...
The Client is used to enqueue a task.
	...
	client := titanqueue.NewClient(redisConnOpt)
	...
	task := titanqueue.NewTask("example", b)

	// Enqueue the task to be processed immediately.
	info, err := client.Enqueue(task)

	// Schedule the task to be processed after one minute.
	info, err = client.Enqueue(t, titanqueue.ProcessIn(1*time.Minute))

The Server is used to run the task processing workers with a given
handler.

	srv := titanqueue.NewServer(redisConnOpt, titanqueue.Config{
	    Concurrency: 10,
	})
...
func (s TaskState) String() string {
	...
	panic("titanqueue: unknown task state")
}
...
func ParseRedisURI(uri string) (RedisConnOpt, error) {
	u, err := url.Parse(uri)
	if err != nil {
		return nil, fmt.Errorf("titanqueue: could not parse redis uri: %w", err)
	}
	switch u.Scheme {
	case "redis", "rediss":
	...
	default:
		return nil, fmt.Errorf("titanqueue: unsupported uri scheme: %q", u.Scheme)
	}
}
...
func parseRedisURI(u *url.URL) (RedisConnOpt, error) {
	...
	if len(u.Path) > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("titanqueue: could not parse redis uri: database number should be the first segment of the path")
		}
	}
	var password string
...
func parseRedisSocketURI(u *url.URL) (RedisConnOpt, error) {
	const errPrefix = "titanqueue: could not parse redis socket uri"
	if len(u.Path) == 0 {
